...
const sectors = [
  { tag: "Principal", title: "Automoción", pattern: "repeating-linear-gradient(45deg, #e8620a 0, #e8620a 1px, transparent 0, transparent 50%)", patternSize: "20px 20px" },
  { tag: "Transporte", title: "Ferroviario", pattern: "radial-gradient(circle at 50% 50%, rgba(232,98,10,0.3) 0%, transparent 60%)" },
  { tag: "Alta Exigencia", title: "Aeronáutico", pattern: "repeating-conic-gradient(rgba(232,98,10,0.08) 0deg, transparent 10deg, transparent 30deg)" },
];
    ...
    <section className="bg-mgbg2 px-6 lg:px-[60px] py-[120px] relative z-[2]">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-end mb-[72px]">
        ...
        <motion.p initial="hidden" whileInView="visible" viewport={{ once: true }} custom={1} variants={revealVariants} className="text-mgsteel text-[0.95rem] leading-relaxed font-light">
          Nuestras piezas se integran en los sistemas más críticos de los principales fabricantes de automóviles y vehículos de transporte de Europa y América.
        </motion.p>
      </div>
        ...
        {sectors.map((sector, i) => (
          <motion.div key={sector.title} custom={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={revealVariants}
            ...
            <div className="absolute inset-0 p-9 flex flex-col justify-end bg-gradient-to-t from-[rgba(10,11,13,0.9)] to-transparent">
              ...
              <h3 className="font-head font-extrabold text-[1.6rem] uppercase tracking-[0.05em] text-foreground">{sector.title}</h3>
            </div>
