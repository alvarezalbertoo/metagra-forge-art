      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.2fr] gap-12 lg:gap-20 items-start">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={revealVariants}>
          ...
          <div className="mt-12 space-y-0">
            {contactDetails.map((detail) => (
              <div key={detail.label} className="flex gap-5 py-6 border-t border-border last:border-b">
        ...
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} custom={2} variants={revealVariants}>
          <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
            ...
            <button type="submit" disabled={sending} className="self-start relative overflow-hidden font-head font-bold text-[0.85rem] tracking-[0.2em] uppercase px-12 py-4 bg-mgaccent text-white hover:-translate-y-0.5 transition-transform group disabled:opacity-60 disabled:cursor-not-allowed">
