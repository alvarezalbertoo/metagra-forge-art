    ...
    <section className="bg-mgbg px-6 lg:px-[60px] py-[120px] relative z-[2]">
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.2fr] gap-12 lg:gap-20 items-start">
        ...
        >
          ...
          <h2
            className="font-head font-extrabold uppercase leading-none tracking-tight text-foreground mb-7"
          ...
          <div className="mt-12 space-y-0">
            {contactDetails.map((detail) => (
              <div key={detail.label} className="flex gap-5 py-6 border-t border-[rgba(255,255,255,0.07)] last:border-b">
                <div className="w-10 h-10 shrink-0 border border-[rgba(255,255,255,0.07)] flex items-center justify-center text-sm text-mgaccent">
                  {detail.icon}
                </div>
                <div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="flex flex-col gap-2">
                ...
                {errors.nombre && <span className="text-[0.7rem] text-red-400">{errors.nombre.message}</span>}
              </div>
              <div className="flex flex-col gap-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="flex flex-col gap-2">
                ...
                {errors.email && <span className="text-[0.7rem] text-red-400">{errors.email.message}</span>}
              </div>
              <div className="flex flex-col gap-2">
            ...
            <div className="flex flex-col gap-2">
              ...
              />
              {errors.mensaje && <span className="text-[0.7rem] text-red-400">{errors.mensaje.message}</span>}
            </div>

            <button
              type="submit"
              className="self-start relative overflow-hidden font-head font-bold text-[0.85rem] tracking-[0.2em] uppercase px-12 py-4.5 bg-mgaccent text-foreground hover:-translate-y-0.5 transition-transform group"
            >
              <span className="absolute inset-0 bg-mgaccent2 -translate-x-full group-hover:translate-x-0 transition-transform duration-350" />
